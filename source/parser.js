...
function getListTag(list) {
    const glyphType = _get(list, ["listProperties", "nestingLevels", 0, "glyphType"]);
...
function getTableCellContent(content) {
    ...
    return content.map(({ paragraph }) => cleanText(getTextFromParagraph(paragraph))).join("");
}
...
function getText(element, { isHeader = false } = {}) {
    ...
    const { link, underline, strikethrough, bold, italic } = element.textRun.textStyle;
...
function getCover(document) {
    ...
    if (!documentStyle || !documentStyle.firstPageHeaderId || !headers[documentStyle.firstPageHeaderId]) {
        return null;
    }

    const headerElement = _get(headers[documentStyle.firstPageHeaderId], ["content", 0, "paragraph", "elements", 0]);
...
function convertGoogleDocumentToJson(document) {
    ...
    body.content.forEach(({ paragraph, table }, i) => {
            ...
            // Headings, Images, Texts
                ...
                paragraph.elements.forEach((el) => {
                    ...
                    // Footnotes
                        ...
                        footnoteIDs[el.footnoteReference.footnoteId] = el.footnoteReference.footnoteNumber;
                    }
                });
        ...
        // Table
            ...
            content.push({
                table: {
                    headers: thead.tableCells.map(({ content }) => getTableCellContent(content)),
                    rows: tbody.map((row) => row.tableCells.map(({ content }) => getTableCellContent(content))),
                },
            });
        }
    });
    ...
    Object.entries(footnotes).forEach(([, value]) => {
        ...
        const text_items = value.content[0].paragraph.elements.map((element) => getText(element));
    ...
    formatedFootnotes.sort((item1, item2) => parseInt(item1.footnote.number) - parseInt(item2.footnote.number));
